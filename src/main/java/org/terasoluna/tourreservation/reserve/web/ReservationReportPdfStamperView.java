...
public class ReservationReportPdfStamperView extends AbstractPdfStamperView {
	...
	protected void mergePdfDocument(Map<String, Object> model, PdfStamper stamper, HttpServletRequest request,
		...
		form.setField("reservedDay", dateFormat.format(downloadPDFOutput.getReservedDay()));
		...
		form.setField("depDay", dateFormat.format(downloadPDFOutput.getDepDay()));
		form.setField("customerName", downloadPDFOutput.getCustomerName());
		form.setField("childUnitPrice", String.valueOf(downloadPDFOutput.getChildUnitPrice()));
		form.setField("depName", downloadPDFOutput.getDepName());
		form.setField("customerBirth", dateFormat.format(downloadPDFOutput.getCustomerBirth()));
		form.setField("arrName", downloadPDFOutput.getArrName());
		form.setField("customerMail", downloadPDFOutput.getCustomerMail());
		form.setField("adultCount", String.valueOf(downloadPDFOutput.getAdultCount()));
		...
		form.setField("printDay", dateFormat.format(downloadPDFOutput.getPrintDay()));
		form.setField("adultPrice", String.valueOf(downloadPDFOutput.getAdultPrice()));
		form.setField("childPrice", String.valueOf(downloadPDFOutput.getChildPrice()));
		form.setField("sumPrice", String.valueOf(downloadPDFOutput.getSumPrice()));
		form.setField("paymentTimeLimit", downloadPDFOutput.getPaymentTimeLimit());
